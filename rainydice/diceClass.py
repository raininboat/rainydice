...
class User(dict):
    ...
    primary key ('U_Platform','U_ID')
        ...
        self.sql_path = sql_path
    # 添加新用户
    def add_user(self,U_Platform,U_ID,U_Name='用户',U_EnabledCard=1,U_Trust=0,U_CriticalSuccess=0,U_ExtremeSuccess=0,U_HardSuccess=0,U_RegularSuccess=0,U_Failure=0,U_Fumble=0):
        ...
        try:
            ...
            pre_sql =''' INSERT INTO user_%d_%d('Card_ID','Card_Name','Card_JSON') VALUES (1,'用户标准人物卡','{}');'''%(U_Platform,U_ID)
            SQL_conn.cursor.execute(pre_sql)
    ...
    def set(self, key, value,platform,user_id):      # 设置用户基本属性，人物卡不在这里
        if user_id not in self[platform]['uset_list']:
            self.add_user(U_Platform=platform,U_ID=user_id)
        self[platform][user_id][key] = value
        #if key not in self.jsonconf:
        sql_path = self.sql_path
        SQL_conn = SQL(sql_path)
        pre_sql = '''UPDATE USER_CONF SET ? = ? WHERE 'U_Platform' IS ? AND 'U_ID' IS ?;'''
        cur = SQL_conn.connection.cursor()
        try:
            cur.execute(pre_sql,(key,value,platform,user_id))
            SQL_conn.connection.commit()
            cur.close()
            if SQL_conn.connection.total_changes == 0:
                self.log(3,'未能更改数据库数据!')
    ...
    def get_card(self,platform , user_id):
        ...
        sql_conn = SQL(self.sql_path)
        pre_sql = '''SELECT * FROM user_%d_%d WHERE 'Card_ID' == 1;'''%(platform,user_id)
        ...
        try:
            ...
            return {'id' : 1,'name' : '用户','data' : {"_null" : -1}}
    def set_card(self,platform , user_id, card_dict ,card_name = None):   # 用st进行人物卡设置
        if user_id not in self[platform]['user_list']:
            self.add_user(U_Platform=platform,U_ID=user_id)
        sql_conn = SQL(self.sql_path)
        pre_sql = '''SELECT * FROM user_%d_%d WHERE 'Card_ID' IS ?;'''%(platform,user_id)
        en_card = self[platform][user_id]['U_EnabledCard']
        try:
            sql_conn.cursor.execute(pre_sql,(en_card))
            temp = sql_conn.cursor.fetchone()
            ...
            card_json = json.dumps(card_dict_all)
            # 进行更新
    ...
    def del_card(self,platform , user_id, card_id = None):   # 用st进行人物卡设置
        ...
        try:
            ...
            if card_id == 1:    # 如果是标准卡1，则特殊操作
                pre_sql = '''INSERT OR REPLACE INTO user_%d_%d ('Card_Name','Card_JSON') VALUES ('用户标准人物卡','{}')'''%(platform,user_id)
